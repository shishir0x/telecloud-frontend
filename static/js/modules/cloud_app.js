...
export function cloudApp(initialIsLoggedIn, isAdmin = true, storageUsed = 0, webdavEnabled = false, webdavUser = '', webdavPassword = '', uploadAPIEnabled = false, uploadAPIKey = '', globalWebdavEnabled = true, globalAPIEnabled = true, webauthnRPID = '', webauthnOrigins = '', initialTheme = 'system', s3Enabled = false, s3AccessKey = '', s3SecretKey = '', globalS3Enabled = true, forceChange = false, logGroupId = '', initialBotTokens = '', initialBotStatuses = '{}', initialTelegramUserId = '', initialBotPoolUploadFolder = '') {
    return {
        ...
        changelog: [],
        latestReleaseUrl: '',
        checkUpdatesEnabled: (window.TeleCloud && window.TeleCloud.checkUpdatesEnabled) || false,
        ...
        get filteredFiles() {
            let results = this.isSearching ? [...this.searchResults] : [...this.files];
        ...
        async performServerSearch(query) {
            ...
            try {
                const res = await fetch(`/api/search?q=${encodeURIComponent(query)}&limit=100&offset=0`, {
                ...
                if (res.ok) {
                    ...
                    console.error('Server search error', res.status);
                }
            } catch (err) {
        ...
        async checkUpdate(force = false) {
            // If automatic update check is disabled in settings, do not perform request unless force=true
            if (!force && !this.checkUpdatesEnabled) {
                return;
            }
            ...
            try {
                ...
                if (res.ok) {
                    const data = await res.json();
                    if (!data.enabled && !force) {
                        return;
                    }
                    if (data.update_available) {
                        this.updateAvailable = true;
                        this.latestReleaseUrl = data.latest_release_url || `https://github.com/${TeleCloud.githubRepo || 'shishir0x/TeleCloud'}/releases`;
                        ...
                        if (dismissedDate !== today) {
                            ...
                            const choice = await this.showUIModal('update', this.t('update_title'), this.t('update_msg') + (latestVer ? ` (${latestVer})` : ''));
                            if (choice === 'confirm') {
                        ...
                        this.showToast('TeleCloud is up to date!', 'success');
                    }
                }
            } catch (e) {
                console.error('Failed to check for updates from backend', e);
                if (force) {
                    this.showToast('Update check unavailable (offline or network error)', 'error');
                }
            } finally {
                this.checkingUpdate = false;
            }
        },
        async toggleCheckUpdates() {
            if (!this.isAdmin) return;
            const newStatus = !this.checkUpdatesEnabled;
            try {
                ...
                if (res.ok) {
                    this.checkUpdatesEnabled = newStatus;
                    ...
                    this.showToast('Failed to save update settings', 'error');
                }
            } catch (e) {
                console.error('Failed to toggle update checking', e);
                this.showToast('Failed to save update settings', 'error');
            }
        },
        async confirmCleanTempFiles() {
            if (!this.isAdmin) return;
            const confirmed = await this.showUIModal(
                'confirm',
                'Clean Temporary Files',
                'Are you sure you want to clean abandoned temporary upload files? Active in-progress uploads are protected and will not be deleted.'
            );
            if (confirmed !== 'confirm') return;

            this.cleaningTemp = true;
            try {
                const res = await fetch('/api/settings/temp/clean', {
                    method: 'POST',
                    headers: { 'X-CSRF-Token': TeleCloud.getCsrfToken() }
                });
                if (res.ok) {
                    const data = await res.json();
                    const count = data.files_cleaned || 0;
                    const bytes = data.formatted_bytes || '0 B';
                    this.showToast(`Cleaned ${count} temporary file(s) (${bytes})`, 'success');
                } else {
                    this.showToast('Failed to clean temporary files', 'error');
                }
            } catch (err) {
                console.error('Failed to clean temp files', err);
                this.showToast('Error cleaning temporary files', 'error');
            } finally {
        ...
        async _doFetchFiles(silentLoad = false) {
            ...
            try {
                ...
                if (!silentLoad) { this.clearSearch(); } else { if (this.currentPage > this.totalPages) this.currentPage = Math.max(1, this.totalPages); }
